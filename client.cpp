...
ulong_t __stdcall Client::init(void* arg) {
	...
	// welcome the user.
	g_notify.add(tfm::format(XOR("welcome to nebula.sbs nigga this jhit loaded 100%"), g_cl.m_user));
...
void Client::DrawHUD() {
	...
	std::string text = tfm::format(XOR("nebula.sbs [beta] | delay: %ims | tick: %i | %s"), ms, rate, time.str().data());
	render::FontSize_t size = render::menu_shade.size(text);

	render::rect_filled(m_width - size.m_width - 18, 8, size.m_width + 8, 2, { g_gui.m_color });
	render::rect_filled(m_width - size.m_width - 18, 8, size.m_width + 8, size.m_height + 2, { 44, 49, 56, 100 });
...
void Client::ClanTag()
{
	...
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("nebula.sbs  "));
	};

	std::string szClanTag = XOR("nebula.sbs  ");
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 3.6) % 18) {
			...
			case 0: {  SetClanTag("n"); break; }
			case 1: {  SetClanTag("ne"); break; }
			case 2: {  SetClanTag("neb"); break; }
			case 3: {  SetClanTag("nebu"); break; }
			case 4: {  SetClanTag("nebul"); break; }
			case 5: {  SetClanTag("nebula"); break; }
			case 6: {  SetClanTag("nebula."); break; }
			case 7: {  SetClanTag("nebula."); break; }
			case 8: {  SetClanTag("nebula.s"); break; }
			case 9: {  SetClanTag("nebula.sb"); break; }
			case 10: { SetClanTag("nebula.sbs"); break; }
			case 11: { SetClanTag("nebula.sb"); break; }
			case 12: { SetClanTag("nebula.s"); break; }
			case 13: { SetClanTag("nebula."); break; }
			case 14: { SetClanTag("nebula."); break; }
			case 15: { SetClanTag("nebula"); break; }
			case 16: { SetClanTag("nebul"); break; }
			case 17: { SetClanTag("nebu"); break; }
			case 18: { SetClanTag("neb"); break; }
			case 19: { SetClanTag("ne"); break; }
			case 20: { SetClanTag("n"); break; }
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(colors::green, XOR("[nebula.sbs] "));
